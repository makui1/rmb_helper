...
参考 Claude Code Desktop，亮色、简洁、现代。

| 元素 | 规格 |
|------|------|
| 整体色调 | 亮色，白/浅灰底 |
| 左侧导航栏 | 浅灰背景（`background-secondary`），0.5px 右边框，宽 180px |
| 强调色 | 橙色 `#D85A30`（选中态、文件图标、执行按钮） |
| 字体 | 系统默认无衬线，13px 正文，12px 辅助文字 |
| 边框 | 0.5px，`color-border-tertiary`（普通），`color-border-secondary`（悬停/强调） |
| 圆角 | 按钮/输入框 `border-radius-md`（8px），卡片 `border-radius-lg`（12px） |
| 日志区 | 等宽字体，浅灰底，绿色成功、橙色警告、灰色完成信息 |
| 实现方式 | QSS 全局样式表，不引入第三方主题库 |
